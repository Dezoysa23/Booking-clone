      ...
      <div className="mx-auto max-w-6xl">
        <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-[#5B6472]">{payments.length} transactions · LKR {totalRevenue.toLocaleString()} revenue</p>
          </div>
        ...
        {payments.length === 0 ? (
          <div className="rounded-2xl bg-white border border-gray-100 shadow-sm p-16 text-center">
            <p className="text-[#5B6472] text-sm">No payment records yet.</p>
          </div>
        ) : (
                ...
                {payments.map((payment) => (
                  <tr key={payment.id} className="hover:bg-[#F8F2E9] transition-colors">
                    <td className="px-5 py-4">
                      ...
                      <p className="text-xs text-[#7C879B] mt-0.5 truncate max-w-[200px]">{payment.user.email}</p>
                    </td>
