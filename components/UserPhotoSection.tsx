...
export default function UserPhotoSection({ propertyId, propertyName, approvedPhotos, isLoggedIn }: Props) {
  ...
  return (
    <section>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="mb-6 rounded-2xl bg-white border border-slate-200/70 shadow-sm p-5 space-y-4">
          <h3 className="font-(family-name:--font-playfair-display) font-semibold text-[#14213d]">Share your photo</h3>
          <p className="text-xs text-slate-500">Your photo will be reviewed before it appears publicly.</p>
          ...
          <div>
            {previewUrl ? (
              <div className="relative">
                ...
                <img src={previewUrl} alt="Preview" className="h-48 w-full object-cover rounded-xl border border-slate-200/70" />
              ...
              <button type="button" onClick={() => fileRef.current?.click()}
                className="w-full flex flex-col items-center gap-2 rounded-xl border-2 border-dashed border-slate-200 bg-slate-50 py-8 text-center hover:border-[#14213d]/30 transition-colors cursor-pointer">
          ...
          <div>
            <label className="mb-1.5 block text-xs font-semibold uppercase tracking-widest text-slate-400">Category</label>
            <select value={category} onChange={(e) => setCategory(e.target.value)}
              className="w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm outline-none focus:border-[#14213d] focus:bg-white">
          ...
          <div>
            <label className="mb-1.5 block text-xs font-semibold uppercase tracking-widest text-slate-400">Caption (optional)</label>
          ...
          <div className="flex gap-3">
            <button type="submit" disabled={submitting || !selectedFile}
              className="rounded-lg bg-[#14213d] px-5 py-2.5 text-sm font-semibold text-white hover:bg-[#1e3a8a] transition-colors disabled:opacity-60">
      ...
      {photos.length > 0 ? (
        <div className="grid gap-3 grid-cols-2 sm:grid-cols-3">
          {photos.map((photo) => {
            ...
            return (
              <div key={photo.id} className="group relative overflow-hidden rounded-xl border border-slate-200/70 bg-white shadow-sm aspect-square">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                ...
                {(photo.caption || cat) && (
                  <div className="absolute inset-x-0 bottom-0 bg-linear-to-t from-black/70 to-transparent p-3 translate-y-full group-hover:translate-y-0 transition-transform duration-200">
        ...
        !showForm && (
          <div className="rounded-xl bg-white border border-slate-200/70 p-8 text-center">
            <span className="material-symbols-outlined text-slate-200 text-4xl">photo_library</span>
            <p className="mt-2 text-sm text-slate-400">No guest photos yet. Be the first to share!</p>
          </div>
        )
      )}
    </section>
